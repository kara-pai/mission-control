...
|-------|------|-------------|
| `delegation_log` | append-only | Every task delegated — who, what, when, status |
| `standup_history` | persistent | Full standup transcripts with action items |
| `system_documentation` | live-updated | Master documentation — always current |
| `workspace_states` | live-updated | All agent workspace states |
| `revenue_products` | persistent | All products built, launched, revenue generated |
| `escalation_log` | persistent | Items escalated to Louis — context, recommendation, resolution |
| `overnight_log` | rolling-24h | Overnight completions, errors, items for Louis |
| `bottleneck_tracker` | persistent | Bottlenecks identified, solutions, elimination status |
| `kara_approval_sop` | versioned | Current approval criteria for Kara's work |
| `crypto_treasury` | time-series | Wallet balances, claims, burns, transfers |
| `codeex_sessions` | live-updated | Active Codeex/Ralph loop sessions — status, location |
...
### Memory Rules for Muddy

1. All memory stores updated in real-time as Muddy operates
...
**Date:** 2026-03-01 01:45 UTC  
**Memory System:** PAR + QMD (Tiago Forte method)  
**Consolidation:** Daily at 02:00  
